...
def showrequest():
    if request.method == 'GET':
        ...
        cursor.execute("SELECT * FROM request WHERE reqby = 'Bahagia'")#jgn lupa diganti
...
def get_request_by_id(id):
    if request.method == 'GET':
        cursor = mysql.connection.cursor()
        query = "SELECT * FROM request WHERE reqby = 'Bahagia' AND idReq=%s" #jgn lupa diganti
...
def tambah_karyawan():
    if request.method == 'POST':
        ...
        cursor = mysql.connection.cursor()
        sql = "INSERT INTO request (reqby, jenis, nama, deskripsi, jumlah, status ) VALUES ('Pamungkas', %s, %s, %s, %s, 'Pending')" #Pamungkas nya jgn lupa diganti
...
def editkaryawan():
    if 'id' in request.args:
        ...
        cursor = mysql.connection.cursor()
        sql = "UPDATE request SET status = %s, appby =%s WHERE idReq = %s"  
        val = (data['status'],data['appby'],request.args['id'])
        cursor.execute(sql, val)
        mysql.connection.commit()
        cursor.close()  
